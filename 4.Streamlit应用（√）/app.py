...
st.set_page_config(
    page_title="TDXd-Related Grade ≥3 Adverse Effect Prediction Model",
    page_icon="🩺",
    layout="wide",
...
def page_single_patient() -> None:
    ...
    col1.metric("Final ensemble probability", f"{prob:.1%}")
    ...
    st.progress(min(max(prob, 0.0), 1.0), text=f"Risk group：{level}；internal Youden threshold：{RISK_THRESHOLD:.3f}")
...
def main() -> None:
    inject_style()
    st.title("TDXd-Related Grade ≥3 Adverse Effect Prediction Model")
    st.caption("Final ensemble: AdaBoost + SVC + SVM_RBF | Outcome: Severe Adverse Effect, AE3_All")
    ...
    st.sidebar.info(
        ...
        f"The current high-risk cutoff is the internal Youden threshold: {RISK_THRESHOLD:.3f}."
    )
    st.sidebar.markdown(
        """
        **Risk group**
        ...
        - Low risk: < 15%
        - Intermediate risk: 15% to the internal Youden threshold
        - High risk: ≥ the internal Youden threshold
        """
    )
