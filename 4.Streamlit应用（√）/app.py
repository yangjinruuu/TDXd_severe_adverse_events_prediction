...
st.set_page_config(
    page_title="严重不良反应（Severe Adverse Effect）预测模型",
    page_icon="🩺",
    layout="wide",
...
def input_form() -> pd.DataFrame | None:
    ...
    with st.form("single_patient_form"):
        ...
        with col3:
            ...
            submitted = st.form_submit_button("预测严重不良反应（Adverse Effect）风险", use_container_width=True)
...
def page_deployment_notes() -> None:
    ...
    st.markdown(
        """
        **本应用用途**：根据 8 个变量预测患者发生 3级及以上严重不良反应（Severe Adverse Effect，AE3_All）的风险。
...
def main() -> None:
    inject_style()
    st.title("严重不良反应（Severe Adverse Effect）预测模型")
