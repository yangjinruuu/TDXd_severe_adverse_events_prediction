...
def inject_style() -> None:
    st.markdown(
        ...
        .guidance-risk {font-size: 1.02rem; font-weight: 650; margin-bottom: 0.8rem;}
...
def clinical_guidance(prob: float) -> list[str]:
    if prob >= HIGH_RISK_CUTOFF:
        return [
            ...
            "Monitoring and prevention: Check complete blood count and relevant laboratory tests before each cycle and consider more frequent monitoring during treatment.",
            "Clinical assessment: Review prior severe toxicity, performance status, metastatic burden, concomitant medications, and baseline organ function.",
            "Supportive care: Assess promptly for fever, infection, bleeding, marked fatigue, respiratory symptoms, or other clinically significant changes.",
            "Treatment planning: If clinically indicated, discuss dose delay, dose modification, supportive treatment, or multidisciplinary review according to the product label and institutional guidance.",
    ...
    if prob >= LOW_RISK_CUTOFF:
        return [
            ...
            "Monitoring: Check complete blood count and relevant laboratory tests before treatment cycles and consider additional follow-up when clinically appropriate.",
            ...
            "Patient education: Explain warning signs such as fever, infection symptoms, unusual bleeding, severe fatigue, respiratory symptoms, or persistent gastrointestinal symptoms.",
            ...
            "The model supports risk stratification and should not be used alone to change treatment.",
        ]
    return [
        ...
        "Reassessment: Reassess risk if new symptoms, laboratory abnormalities, treatment changes, or clinical deterioration occur.",
...
def risk_presentation(prob: float) -> tuple[str, str, str, str]:
    if prob >= HIGH_RISK_CUTOFF:
        return "⚠️", "High Risk of TDXd-related severe adverse events", "risk-high", "🔴"
    if prob >= LOW_RISK_CUTOFF:
        return "⚠️", "Intermediate Risk of TDXd-related severe adverse events", "risk-intermediate", "🟡"
    return "✅", "Low Risk of TDXd-related severe adverse events", "risk-low", "🟢"
...
def input_form() -> pd.DataFrame | None:
    ...
    with st.form("single_patient_form"):
        ...
        treatment_1, treatment_2, treatment_3 = st.columns(3)
        with treatment_1:
        ...
        metastasis_1, metastasis_2, metastasis_3 = st.columns(3)
        with metastasis_1:
        ...
        st.markdown("")
        button_spacer_1, button_spacer_2, button_col = st.columns([1, 1, 1])
        with button_col:
...
def main() -> None:
    ...
    st.markdown("<div class='page-subtitle'>Predicting severe adverse events in patients receiving trastuzumab deruxtecan (T-DXd) therapy</div>", unsafe_allow_html=True)

    st.sidebar.header("About")
    st.sidebar.markdown(
        "<div class='about-card'>After entering the relevant patient parameters, the system estimates the probability of TDXd-related severe adverse events.</div>",
        unsafe_allow_html=True,
    )
